...
parser.add_argument("--scale", type=float, default=1024.0)
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(train_dataloader)):
        ...
        conv_train_data.append(torch.sum(conv_spikes["Y"].get("s").view(time, -1).int(), 1).tolist())
...
pre_average = []
conv_classes = []
encoder = PoissonEncoder(time=encoding_time, dt=encoding_dt)
...
for line in whole_data:
    ...
    encoded = encoder.enc(datum=converted, time=encoding_time, dt=encoding_dt)
...
preprocessed = whole_data[whole_data[:, time].argsort()]
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(test_dataloader)):
        ...
        conv_test_spikes.append(torch.sum(conv_spikes["Y"].get("s").view(time, -1).int(), 1).tolist())
        conv_test_labels.append(label.tolist())

        network_conv.reset_state_variables()  # Reset state variables.

conv_test_spikes = np.array(conv_test_spikes)
