...
data = minmax_scale(load_digits().data)
...
sorted = whole[whole[:, -1].argsort()]

for line in sorted:
    data_sample = line[0:len(line) - 1]
    marker = line[-1]

    preprocessed.append(data_sample)
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(train_data)):
        ...
        inputs = {"X": batch["encoded_image"].view(int(time / dt), 1, 8, 8, 1)}
        if gpu:
        ...
        if train_plot:
            ...
            inpt = inputs["X"].view(time, train_data[-1]["encoded_image"].shape[1]).sum(0).view(8, 8)
            input_exc_weights = network.connections[("X", "Ae")].w
            square_weights = get_square_weights(
                input_exc_weights.view(train_data[-1]["encoded_image"].shape[1], n_neurons), n_sqrt, 8,
            )
...
for step, batch in enumerate(test_data):
    ...
    if test_plot:
        image = batch["encoded_image"].view(num_inputs, time)
        inpt = inputs["X"].view(time, test_data[-1]["encoded_image"].shape[1]).sum(0).view(8, 8)
        spikes_ = {layer: spikes[layer].get("s") for layer in spikes}
