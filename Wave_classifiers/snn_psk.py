...
fname = "D:/SNN_dataset/Demodulation/6Mbps/"\
        "802.11_demodulation_6Mbps_21dB_10000_0.txt"
...
for line in raw:
    ...
    com = complex(line_data[0], line_data[1])
    phase = np.angle(com) + np.pi
    decode_phase = np.zeros(4)
    decode_phase[int(phase * 2 / np.pi)] = 1

    scaled = np.array(decode_phase) * intensity
