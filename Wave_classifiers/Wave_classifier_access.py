...
parser.add_argument("--intensity", type=float, default=16)
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(train_data)):
        ...
        print("input spikes", spikes["X"].get("s").view(-1).long().sum(0).item())
        print("exc spikes", spikes["Ae"].get("s").view(-1).long().sum(0).item())
